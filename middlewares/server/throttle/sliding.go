...
func (SlidingWindow) New(cfg Config) frame.HandlerFunc {
	var (
		// Limiter variables
		mux        = &sync.RWMutex{}
	...
	return func(ctx context.Context, c *frame.Context) {
		...
		if remaining < 0 {
			...
			c.Set(consts.HeaderRetryAfter, strconv.FormatUint(resetInSec, 10))

			// Call LimitReached handler
			cfg.LimitReached(ctx, c)
			return
		}
		...
		if (cfg.SkipSuccessfulRequests && c.Response.StatusCode() < consts.StatusBadRequest) ||
			(cfg.SkipFailedRequests && c.Response.StatusCode() >= consts.StatusBadRequest) {
			mux.Lock()
			e.currHits--
			remaining++
			mux.Unlock()
